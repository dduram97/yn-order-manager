...
interface AllCustomersPickerProps {
...
export function AllCustomersPicker({
  templateType,
  disabled = false,
  onSelect,
}: AllCustomersPickerProps) {
  ...
  const loadCustomers = useCallback(async (keyword: string) => {
    ...
    try {
      ...
      const json: AllCustomersApiResponse = await res.json();
  ...
  return (
    <>
      <button
        ...
        disabled={disabled}
        onClick={handleOpen}
        className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50 disabled:opacity-50"
      >
        기존 고객 불러오기
      </button>
      ...
      {open && (
        ...
        >
          ...
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="all-customers-title"
                  ...
                  {customers.map((customer) => (
                    <li key={customer.id}>
                      <button
                        ...
                        className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition hover:bg-zinc-50 active:bg-zinc-100"
                      >
